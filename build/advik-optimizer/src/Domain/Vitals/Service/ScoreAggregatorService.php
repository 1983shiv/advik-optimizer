...
class ScoreAggregatorService {
	...
	public function latestMetricsWithTimestamps( string $device = 'mobile' ): array {
		$result = [];
		foreach ( [ 'lcp', 'cls', 'inp', 'ttfb' ] as $type ) {
			$row = $this->repository->getLatestMetricWithTimestamp( $type, $device, 'lab' )
				?? $this->repository->getLatestMetricWithTimestamp( $type, $device, 'field' );
