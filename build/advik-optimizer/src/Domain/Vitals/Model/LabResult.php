...
class LabResult {
	...
	public function toMetrics(): array {
		...
		if ( 0.0 !== $this->lcp ) {
			$metrics[] = new VitalMetric( null, $hash, $this->url, 'lcp', $this->lcp, $this->device, 'lab', $this->recordedAt );
		}
		$metrics[] = new VitalMetric( null, $hash, $this->url, 'cls', $this->cls, $this->device, 'lab', $this->recordedAt );
		if ( 0.0 !== $this->inp ) {
			$metrics[] = new VitalMetric( null, $hash, $this->url, 'inp', $this->inp, $this->device, 'lab', $this->recordedAt );
		}
		if ( 0.0 !== $this->ttfb ) {
			$metrics[] = new VitalMetric( null, $hash, $this->url, 'ttfb', $this->ttfb, $this->device, 'lab', $this->recordedAt );
		}
		$metrics[] = new VitalMetric( null, $hash, $this->url, 'performance', (float) $this->performanceScore, $this->device, 'lab', $this->recordedAt );
		$metrics[] = new VitalMetric( null, $hash, $this->url, 'seo', (float) $this->seoScore, $this->device, 'lab', $this->recordedAt );
		$metrics[] = new VitalMetric( null, $hash, $this->url, 'accessibility', (float) $this->accessibilityScore, $this->device, 'lab', $this->recordedAt );
		$metrics[] = new VitalMetric( null, $hash, $this->url, 'best_practices', (float) $this->bestPracticesScore, $this->device, 'lab', $this->recordedAt );
